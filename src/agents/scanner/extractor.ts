...
export async function extractSecurityData(
  ...
  const linkRegex = /href=["']([^"']*(?:login|signin|signup|register|auth)[^"']*)["']/gi;
  let linkMatch;
  ...
  if (htmlLower.includes('google') && (htmlLower.includes('sign in') || htmlLower.includes('oauth'))) {
    authFlow.oauthProviders.push('Google');
  }
  if (htmlLower.includes('facebook') && (htmlLower.includes('sign in') || htmlLower.includes('oauth'))) {
    authFlow.oauthProviders.push('Facebook');
  }
  if (htmlLower.includes('github') && (htmlLower.includes('sign in') || htmlLower.includes('oauth'))) {
    authFlow.oauthProviders.push('GitHub');
  }
